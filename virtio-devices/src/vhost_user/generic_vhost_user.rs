...
impl GenericVhostUser {
    ...
    pub fn new(
        ...
        // Create the bounce pool up front, sized for the largest queue.
        let bounce = match bounce {
            Some(cfg) => {
                let max_queue_size = request_queue_sizes.iter().copied().max().unwrap_or(0);
                Some(BounceState::new(&cfg, num_queues, max_queue_size).map_err(Error::Bounce)?)
            }
            None => None,
        };
        ...
        let (
            ...
            let avail_features = if bounce.is_some() {
                mask_bounce_features(super::DEFAULT_VIRTIO_FEATURES)
            } else {
                super::DEFAULT_VIRTIO_FEATURES
            };
