...
mod tests {
    ...
    fn blk_avail_features_masks_ring_bits_when_bounce() {
        ...
        // Multi-queue keeps the MQ bit; in-order is masked, but indirect
        // descriptors and event-idx remain supported (pool-side tables and
        // worker-managed event fields respectively).
        assert_ne!(bounced & (1 << VIRTIO_BLK_F_MQ), 0);
        assert_eq!(bounced & (1 << crate::VIRTIO_F_IN_ORDER), 0);
        assert_ne!(bounced & (1 << crate::VIRTIO_F_RING_INDIRECT_DESC), 0);
        assert_ne!(bounced & (1 << crate::VIRTIO_F_RING_EVENT_IDX), 0);
    }
